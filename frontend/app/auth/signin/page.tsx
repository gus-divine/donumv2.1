...
export default function SignInPage() {
  ...
  useEffect(() => {
    if (!authLoading && session && role) {
      // Use redirect URL if provided (e.g. from subdomain), otherwise use role-based default
      const defaultRedirect = (() => {
        if (role === 'donum_member') return '/user/member/dashboard';
        if (role === 'donum_prospect') return '/user/prospect/dashboard';
        if (['donum_staff'].includes(role)) return '/team/dashboard';
        if (['donum_admin', 'donum_super_admin'].includes(role)) return '/admin/dashboard';
        if (role === 'donum_partner') return '/partners/dashboard';
      ...
      if (role === 'donum_prospect') {
        checkPrequalificationStatus(defaultRedirect);
        return;
      }
  ...
  const checkPrequalificationStatus = async (defaultRedirect: string) => {
    try {
      ...
      if (isComplete) {
        ...
        router.push('/user/prospect/prequalify');
      }
    } catch (error) {
