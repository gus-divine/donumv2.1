...
export default function MemberDashboardPage() {
  ...
  const { setApplicationContextFromPage } = useChatPanel();
  ...
  // Register application context for chat (members have applications from when they were prospects)
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    getApplications({ applicant_id: user.id })
      .then((apps) => {
        if (cancelled) return;
        const latest = apps
          .filter((a) => a.application_type !== 'prequalification')
    ...
    <main className="min-h-screen bg-gradient-to-br from-[var(--background)] via-[var(--surface)]/30 to-[var(--background)] p-8">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 border-l-4 border-red-500 pl-4 py-2">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <button
                  onClick={() => router.push('/user/member/payments')}
        ...
        <div className="mb-8 pt-4 border-t-2 border-[var(--core-gold)] pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <button
              onClick={() => router.push('/user/member/loans')}
            ...
            <button
              onClick={() => router.push('/user/member/documents')}
              className="flex items-center gap-3 p-4 border-l-4 border-[var(--core-blue)] hover:bg-[var(--surface-hover)] transition-colors text-left group"
            >
            ...
            <button
              onClick={() => router.push('/user/member/profile')}
              className="flex items-center gap-3 p-4 border-l-4 border-[var(--core-gold)] hover:bg-[var(--surface-hover)] transition-colors text-left group"
            >
          ...
          {loans.length === 0 ? (
            ...
            <div className="space-y-4">
              {loans.slice(0, 3).map((loan) => (
                <div
                  key={loan.id}
                  onClick={() => router.push(`/user/member/loans/${loan.id}`)}
