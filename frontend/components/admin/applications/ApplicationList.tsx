          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[var(--border)]">
                ...
                <th className="text-left p-4 text-sm font-semibold text-[var(--text-primary)]">Type</th>
                <th 
                  className="text-left p-4 text-sm font-semibold text-[var(--text-primary)] cursor-pointer hover:bg-[var(--surface-hover)] select-none"
                  ...
                  >
                    <td className="p-4">
                      <div className={`flex items-center gap-2 ${isGroupedApp ? 'pl-6' : ''}`}>
                        {hasMoreApps && (
                          <button
                            onClick={(e) => { e.stopPropagation(); applicantId && toggleApplicantExpansion(applicantId); }}
                        ...
                        {isExpanded && applicantId && isFirstInGroup && (
                          <button
                            onClick={(e) => { e.stopPropagation(); toggleApplicantExpansion(applicantId); }}
                            className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {canEdit('/admin/applications') && application.status === 'submitted' && (
                        <>
                          <button
                            onClick={(e) => { e.stopPropagation(); handleQuickApprove(application.id); }}
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); handleQuickReject(application.id); }}
                            disabled={processingAppId === application.id}
                      ...
                      {canDelete('/admin/applications') && (
                        <button
                          onClick={(e) => { e.stopPropagation(); handleDeleteClick(application.id, application.application_number); }}
